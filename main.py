...
<div id='modeBox' class='mode'>SCALPING - At Support!</div>
<div class='card'><div style='display:flex;justify-content:space-between'><small>Live (<span id='chartLabel'>EURUSD D1</span>) <span class='verified'>FIXED</span></small><small id='livePrice' style='color:#ffaa00;font-weight:bold'>1.13667</small></div><canvas id='chart' height='350' style='width:100%;background:#000;border-radius:10px;margin-top:8px'></canvas>
<div class='tps'><div class='tpbox' style='border-color:#00ff88'><div style='font-size:9px;color:#00ff88'>ENTRY</div><b id='entryTxt' style='color:#00ff88'>1.13667</b></div><div class='tpbox' style='border-color:#ff4444'><div style='font-size:9px;color:#ff4444'>SL</div><b id='slTxt' style='color:#ff4444'>1.13454</b></div><div class='tpbox' style='border-color:#ffff00'><div style='font-size:9px;color:#ffff00'>TP1</div><b id='tp1Txt' style='color:#ffff00'>1.13817</b></div></div>
...
<div id='storyBox' style='margin-top:10px;background:#000;padding:10px;border-radius:8px;border:1px solid #00ff88;font-size:11px;color:#00ff88'>FIXED: EURUSD 1.13667 matches MT5 (was 0.80389 bug)!</div></div>
...
function draw(d){let c=document.getElementById('chart'),x=c.getContext('2d'); c.width=c.clientWidth; c.height=350; x.clearRect(0,0,c.width,c.height); let cs=d.candles; let min=Math.min(...cs.map(v=>v.l), d.support, d.resistance)*0.998; let max=Math.max(...cs.map(v=>v.h), d.support, d.resistance)*1.002; let range=max-min; let h=c.height-40; let cw=c.width/cs.length*0.6; cs.forEach((k,i)=>{let px=(i/(cs.length-1))*c.width; let oY=h - ((k.o-min)/range*h); let cY=h - ((k.c-min)/range*h); let hY=h - ((k.h-min)/range*h); let lY=h - ((k.l-min)/range*h); let green=k.c>=k.o; x.strokeStyle=green?'#00ff88':'#ff4444'; x.lineWidth=1; x.beginPath(); x.moveTo(px,hY); x.lineTo(px,lY); x.stroke(); x.fillStyle=green?'#00ff88':'#ff4444'; let top=Math.min(oY,cY); let hg=Math.max(2,Math.abs(oY-cY)); x.fillRect(px-cw/2,top,cw,hg);});}
