...
def calc_tps(entry,sup,dec):
    ...
    # Pips calc: dec 5 or 4 => 0.0001 = 1 pip, dec 2 => 0.01? but for BTC we use points
    pip_factor=10000 if dec>=4 else 100 if dec==2 else 10000
    sl_pips=dist*pip_factor
    tp1_pips=(tp1-entry)*pip_factor; tp2_pips=(tp2-entry)*pip_factor; tp3_pips=(tp3-entry)*pip_factor
    return tp1,tp2,tp3,dist,sl_pips,tp1_pips,tp2_pips,tp3_pips
...
def cnd():
    ...
    latest["chat"].append(f"[{sa_time.strftime('%H:%M:%S')}] {latest['selected']} ENTRY {new_p:.{latest['dec']}f} SL {sl_p:.0f}pips TP1 250% {tp1_p:.0f}pips")
    if len(latest["chat"])>15: latest["chat"].pop(0)
    return jsonify({"candles":candles,"price":new_p,"support":latest["support"],"resistance":latest["resistance"],"entry":new_p,"sl":latest["support"],"tp1":tp1,"tp2":tp2,"tp3":tp3,"dist":dist,"sl_pips":sl_p,"tp1_pips":tp1_p,"tp2_pips":tp2_p,"tp3_pips":tp3_p,"signal":f"BUY at Support VERIFIED {latest['selected']}","mode":"SCALPING - At Support Critical!","chat":latest["chat"],"selected":latest["selected"],"timeframe":latest["tf"],"dec":latest["dec"],"name":latest["name"],"live_price":new_p})
...
def ana():
    try:
        ...
        name=BASE.get(m,{}).get("name",m); fmt=f"{{:.{dec}f}}"
        analysis=f"""✅ ANALYZED: {m} {name}
LIVE: {fmt.format(p)}

SL: {fmt.format(sup)} = {sl_p:.0f} pips (100%)
...
<div id='sig' class='signal'>BUY at Support VERIFIED - TPs with PIPS</div>
<div id='modeBox' class='mode'>RR 1:2.5 (250%) | 1:3.85 (385%) | 1:5.8 (580%)</div>
<div class='card'><div style='display:flex;justify-content:space-between'><small>Live (<span id='chartLabel'>USDZAR D1</span>) <span class='verified'>PIPS FIXED</span></small><small id='livePrice' style='color:#ffaa00;font-weight:bold'>16.4192 LIVE</small></div><canvas id='chart' height='500' style='width:100%;background:#fff;border-radius:10px;margin-top:6px'></canvas>
<div class='tps'><div class='tpbox' style='border-color:#00ff88'><div style='font-size:8px;color:#00ff88'>ENTRY</div><b id='entryTxt' style='color:#00ff88'>16.4192</b><div id='entryPips' style='font-size:9px;color:#888'></div></div><div class='tpbox' style='border-color:#ff4444'><div style='font-size:8px;color:#ff4444'>SL (100%)</div><b id='slTxt' style='color:#ff4444'>16.2000</b><div id='slPips' style='font-size:9px;color:#ff4444;font-weight:bold'>2192 pips</div></div><div class='tpbox' style='border-color:#ffff00'><div style='font-size:8px;color:#ffff00'>TP1 250%</div><b id='tp1Txt' style='color:#ffff00'>16.96</b><div id='tp1Pips' style='font-size:9px;color:#ffff00;font-weight:bold'>5480 pips</div></div></div>
<div class='tps' style='margin-top:4px'><div class='tpbox' style='border-color:#ffaa00'><div style='font-size:8px;color:#ffaa00'>TP2 385%</div><b id='tp2Txt' style='color:#ffaa00'>17.26</b><div id='tp2Pips' style='font-size:9px;color:#ffaa00;font-weight:bold'>8439 pips</div></div><div class='tpbox' style='border-color:#00aaff'><div style='font-size:8px;color:#00aaff'>TP3 580%</div><b id='tp3Txt' style='color:#00aaff'>17.69</b><div id='tp3Pips' style='font-size:9px;color:#00aaff;font-weight:bold'>12713 pips</div></div><div class='tpbox'><div style='font-size:8px'>RESIST</div><b id='resTxt' style='color:#ff4444'>16.6000</b><div id='distPips' style='font-size:9px;color:#888'>RR 1:2.5/3.85/5.8</div></div></div>
<div id='storyBox' style='margin-top:8px;background:#000;padding:8px;border-radius:6px;border:1px solid #00ff88;font-size:10px;color:#00ff88'>Pips showing</div></div>
...
function up(){
  ...
  fetch('/analyze',{method:'POST',body:fd}).then(r=>r.json()).then(d=>{
    resDiv.innerText=d.analysis; btn.innerText='✅ ANALYZED!'; setTimeout(()=>{btn.innerText='🔍 ANALYZE'; btn.disabled=false;},2000);
  }).catch(err=>{
...
function tick(){fetch('/candles').then(r=>r.json()).then(d=>{
  let dec=d.dec; let f=(v)=>Number(v).toFixed(dec);
  document.getElementById('sel').innerText=d.selected; document.getElementById('selTF').innerText=d.timeframe;
  document.getElementById('chartLabel').innerText=d.selected+' '+d.timeframe;
  document.getElementById('livePrice').innerText=f(d.price)+' LIVE'; document.getElementById('topPrice').innerText=f(d.price);
  document.getElementById('entryTxt').innerText=f(d.entry); document.getElementById('slTxt').innerText=f(d.sl);
  document.getElementById('tp1Txt').innerText=f(d.tp1); document.getElementById('tp2Txt').innerText=f