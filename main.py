...
def update_prices():
    ...
    # JSE + DERIV
    latest["JSE_TOP40"]+=random.uniform(-25,25)
    latest["R_75"]+=random.uniform(-3,3)
...
def analyze():
    try:
        f=request.files.get('image'); from PIL import Image
        ...
        return jsonify({"signal":sig,"trend":trend,"confidence":conf,"market":"ALL","reason":"Scalping setup detected for FOREX/CRYPTO/JSE/DERIV"})
    except Exception as e: return jsonify({"signal":"Error","trend":str(e)[:50],"confidence":0,"reason":"error"})
